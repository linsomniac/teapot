...
describe('selector stepping (§8.3/§10)', () => {
  it('emits one step per full ±1.0 accumulated lanes', () => {
    const { sim, s } = levelSelectSim(); // opens at 1
  ...
  it('release clears the accumulator — no post-release backlog', () => {
    ...
    expect(s.selector).toBe(1); // nothing emitted after release
  ...
  it('clamps at 1 and at max(9, maxLevelReached) — no wrap', () => {
    const { sim, s } = levelSelectSim(9); // max is 9; opens at 1
    for (let i = 0; i < 300; i++) sim.tick(makeInput({ move: 0.45 }));
    expect(s.selector).toBe(9); // walked up and clamped at the top
    for (let i = 0; i < 300; i++) sim.tick(makeInput({ move: -0.45 }));
  ...
  it('LEVEL_SELECT entry initializes selector to 1 and clears the accumulator', () => {
    ...
    expect(s.selector).toBe(1); // opens at 1 regardless of maxLevelReached (§10)
    expect(s.selectorAccum).toBe(0);
  });
});
